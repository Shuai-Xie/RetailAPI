...
def cvt_poly_fpts_to_xywh(points, img_w, img_h):
    ...
    xmin, xmax = int(np.min(poly_pts[:, 0])), int(np.max(poly_pts[:, 0]))  # np.int -> int, json serializable
    ymin, ymax = int(np.min(poly_pts[:, 1])), int(np.max(poly_pts[:, 1]))
    # judge in image
...
def cvt_poly_fpts_to_segmentation(points, img_w, img_h):
    poly_pts = np.array([pt_float2int(pt, img_w, img_h) for pt in points]).reshape((1, -1)).tolist()
    return poly_pts
